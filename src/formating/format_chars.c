 ...
 * On error, return -1
 */
char *format_char(va_list c, __attribute__((unused)) printing_format * format)
{
...
/**
 * format_str - writes the character c to stdout
 * @c: an arg list containing the string to be printed
 * @format: the format to use to prepare the string
 * Return: On success the lenght of the string printed
 * -1 on failure
 */
char *format_str(va_list c, __attribute__((unused))printing_format * format)
{
